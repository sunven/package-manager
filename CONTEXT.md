...
一个本机开发环境观察与维护工具。它把多个互不相干的包管理器（npm、pnpm、Yarn、nvm、Homebrew、Maven、pip、Cargo、Docker、Bun、uv）、本地项目派生数据、编辑器工作区数据和代理留下的会话记录收敛成一套统一词汇，并在此之上提供受约束的维护操作。
...
**会话记录 (Session Record)**:
代理 CLI 留在各自那一个 home 里的单次对话普通文件。Codex 与 Claude 的会话记录彼此独立，观察与移除也独立于项目清理；内容不能由本工具或管理器重建。
_Avoid_: 会话、缓存、项目派生数据、会话清理、扫描根目录、符号链接

**最后活动时间 (Last Activity)**:
会话记录内容里出现的最晚时刻。它表示这段对话最后一次有活动，与文件修改时间无关。
_Avoid_: 修改时间、创建时间

**工作目录 (Working Directory)**:
会话记录内容里记下的这段对话发生的位置。没有工作目录的会话记录仍是会话记录。
_Avoid_: 项目位置、项目目录、扫描根目录

**使用中 (In Use)**:
文件仍被其他进程打开的会话记录。会话记录移除不包含使用中的会话记录。
_Avoid_: 活跃会话、当前会话
...
**会话记录移除 (Session Record Removal)**:
把同一来源的会话记录移入废纸篓的操作，清空废纸篓前仍可恢复。动手时重判资格：按年龄的要看最后活动时间是否仍早于门槛且未在使用中；用户明确选中的一条或多条，要看它是否仍是未在使用中的会话记录。最后活动时间更新不取消明确选中的移除。单个失败不阻止其余记录。
_Avoid_: 清理、永久删除、删除会话
