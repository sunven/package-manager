import type { MaintenanceResult, NpmMaintenanceRequest } from "../state";
...
export function MaintenanceConfirmationBanner({
  ...
  const confirmationCopy = confirmation.kind === "cleanCache"
    ? {
      title: "确认清理 npm 缓存",
    ...
    : {
      title: "确认卸载 npm 全局包",
      description: `将执行 npm uninstall -g ${confirmation.packageName}。卸载后会刷新 npm 包列表。`,
      confirm: "卸载",
    };
...
function maintenanceResultCopy(request: NpmMaintenanceRequest, result: MaintenanceResult) {
  ...
  return result.tone === "bad"
    ...
    : {
      title: "npm 全局包已卸载",
      description: `${request.packageName} 已卸载，npm 包列表已刷新。`,
    };
}
