import { Activity, Code2, List, MessagesSquare, Moon, RefreshCw, Settings, Sun, Trash2 } from "lucide-react";
...
const viewMeta: Record<ViewId, { code: string; label: string }> = {
  ...
  sessions: { code: "SESSION.LOG / 06", label: "会话记录" },
      ...
      <div className="telemetry-frame">
        <header>
          <div className="telemetry-rail">
            ...
            <samp className="truncate">{activeView === "cleanup" ? "PROJECT DATA CHANNEL" : activeView === "vscode" || activeView === "sessions" ? activeMeta.code : scanMeta || activeMeta.code}</samp>
            ...
            <div className="telemetry-control-block">
              <dl className="telemetry-readouts">
                <div className="telemetry-readout">
                  <dt className="telemetry-readout-label">{activeView === "vscode" || activeView === "sessions" ? "PACKAGE FOOTPRINT / 管理器占用" : "TOTAL FOOTPRINT / 总占用"}</dt>
              ...
              {activeView === "cleanup" || activeView === "vscode" || activeView === "sessions" ? (
                <div className="flex min-h-12 items-center bg-card px-4 text-xs text-muted-foreground">
                  <samp>{activeView === "vscode" ? "[ WORKSPACE STORAGE ]" : activeView === "sessions" ? "[ SESSION RECORDS ]" : "[ PROJECT SCAN ]"}</samp>
                </div>
              ) : (
                <Button
